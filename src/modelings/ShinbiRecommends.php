...
trait ShinbiRecommends
{
	public function create_edit_model(&$row, $clinics = null)
	{
		...
		if (preg_match("/shinbi/",filter_input(INPUT_SERVER, 'SERVER_NAME', FILTER_SANITIZE_STRING))) {
	...
	private function get_description_string($row, $clinics = null)
	{
		...
		$str .= 'この記事では、';
		$str .= (!empty($row->station_name)) ? $row->station_name.'駅で' : $this->get_pref_city_string($row).'で';
		$str .= $row->attribute_name.'に対応しているおすすめの歯医者さんをご紹介します。';

		$str .= 'アクセスや診療時間などの基本情報や医院ごとの特長を掲載していますので、';
		$str .= (!empty($row->station_name)) ? $row->station_name.'駅で' : $this->get_pref_city_string($row).'で';
		$str .= $row->attribute_name.'をお考えの方は是非参考にしてください。';
	...
	private function get_lead_string($row, $clinics = null)
	{
		...
		$str .= $row->attribute_name.'ができるおすすめの歯医者さんをご紹介します。';

		$str .= (!empty($row->station_name)) ? $row->station_name.'駅周辺で' : $this->get_pref_city_string($row).'で';
		$str .= $row->attribute_name.'をするならどこのクリニック？何に注目して選ぶべき？そんなお悩みがある方に本記事はおすすめです。&#10;&#10;';

		$str .= 'この記事で紹介する';
		$str .= (!empty($row->station_name)) ? $row->station_name.'駅周辺で' : $this->get_pref_city_string($row).'で';
		$str .= $row->attribute_name.'ができるおすすめの歯科医院は下記の通りです。&#10;&#10;';
		...
		$str .= $row->attribute_name.'対応の歯医者さん選びの参考として、アクセスや診療時間などの基本情報や医院ごとの特長、院内写真などを掲載しています。&#10;';
		$str .= $row->attribute_name.'を検討中の方はぜひ読んでみてください。';
