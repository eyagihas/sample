...
{
	public function create_edit_model(&$row, $clinics)
	{
		...
		if (preg_match("/kyousei/",filter_input(INPUT_SERVER, 'SERVER_NAME', FILTER_SANITIZE_STRING))) {
    ...
    private function get_alt_string($row)
	{
		...
		$str.= $this->get_pref_city_string($row);
		if ($row->attribute_pathname !== 'orthodontics') {
			$str.= ' '.$row->attribute_name.'対応';
		}
		$str.= ' おすすめしたい矯正歯科';
	...
	private function get_description_string($row, $clinics = null)
	{
		...
		$str .= ($row->attribute_pathname !== 'orthodontics') ?
		$row->attribute_name.'ができる' : '矯正歯科治療ができる';

		$str .= 'おすすめの歯医者さん';
		$str .= ($row->attribute_pathname !== 'orthodontics') ? '（矯正歯科）' : '';
		$str .= 'をご紹介します。';
		...
		$str .= ($row->attribute_pathname !== 'orthodontics') ? $row->attribute_name : '歯列矯正';
	...
	private function get_lead_string($row, $clinics = null)
	{
		...
		$str .= 'この記事で紹介する';
		$str .= (!empty($row->station_name)) ? $row->station_name.'駅周辺で' : $this->get_pref_city_string($row).'で';
		$str .= ($row->attribute_pathname !== 'orthodontics') ? $row->attribute_name.'に対応している' : '';
		...
		$str .= ($row->attribute_pathname !== 'orthodontics') ? $row->attribute_name : '矯正歯科治療';
    ...
    private function get_lead_clinics_string($clinics)
    {
    	$str = '';

    	$str.= '<ul class="mainContents__clinic-wrapper">';

    	foreach ($clinics as $clinic) {
    		$str.= '<li>'.$clinic->clinic_name.'</li>';
    	}
