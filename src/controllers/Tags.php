...
class Tags extends Base
{
	...
	protected function show_cms_list()
	{
		...
		$list->data = \Services\Factory::get_instance('tag')->get_cms_list($request,$page,$limit);
		$list->total = \Services\Factory::get_instance('tag')->get_total_count($request);
	...
	protected function show_add_form()
	{
		...
		$value = new \stdClass();
		$value = $this->get_values($site, ['tag_id' => 0]);

		/* 出力 */
		\Services\Render::render($this->_view, $this->_response, 'tag/add_form.twig', [
			'meta' => $this->get_meta(),
			'header' => $header,
			'value'  => $value
		]);
	...
	protected function add()
	{
		...
		\Services\Factory::get_instance('tag')->insert($request);
	...
	protected function update()
	{
		...
		\Services\Factory::get_instance('tag')->update($request);
