...
/* インプラントネットプラスCMS おすすめページ */
class ImplantRecommends extends Base
{
	use \Values\Meta;
	use \Values\ImplantRecommends;
	...
	protected function show_cms_list()
	{
		...
		$list->data = \Services\Factory::get_instance('implant_recommend')->get_cms_list($request,$page,$limit);
		$list->total = \Services\Factory::get_instance('implant_recommend')->get_total_count($request);
	...
	protected function show_select_form()
	{
		...
		$list->attributes = \Services\Factory::get_instance('attribute')->get_list(['type'=>'implant', 'is_13'=>true, 'is_valid'=>true, 'attribute_type'=>1]);
	...
	protected function show_select_form_type2()
	{
		$list = new \stdClass();
		$list->prefectures = \Services\Factory::get_instance('prefecture')->get_all(['is_domestic'=>true]);
		$list->attributes = \Services\Factory::get_instance('attribute')->get_list(['type'=>'implant', 'is_valid'=>true, 'attribute_type'=>2]);
	...
	protected function show_cms_detail()
	{
		...
		$clinics = \Services\Factory::get_instance('implant_recommend_clinic')->get_list($recommend_id, false, true);
		foreach ($clinics as $clinic) {
			$features = \Services\Factory::get_instance('implant_recommend_clinic_feature')->get_cms_by_clinic_id($recommend_id, $clinic->clinic_id, false);
			$clinic->feature_title = $this->implode_data($features, 'feature_title');
			$clinic->feature_text = $this->implode_data($features, 'feature_text');

			$flows = \Services\Factory::get_instance('implant_recommend_clinic_flow')->get_cms_by_clinic_id($recommend_id, $clinic->clinic_id, false);
			$clinic->flow_title = $this->implode_data($flows, 'flow_title');
			$clinic->flow_text = $this->implode_data($flows, 'flow_text');
		}

		$preview_clinics = \Services\Factory::get_instance('implant_recommend_clinic')->get_list($recommend_id, true, true);
		foreach ($preview_clinics as $clinic) {
			$features = \Services\Factory::get_instance('implant_recommend_clinic_feature')->get_cms_by_clinic_id($recommend_id, $clinic->clinic_id, true);
			$clinic->feature_title = $this->implode_data($features, 'feature_title');
			$clinic->feature_text = $this->implode_data($features, 'feature_text');

			$flows = \Services\Factory::get_instance('implant_recommend_clinic_flow')->get_cms_by_clinic_id($recommend_id, $clinic->clinic_id, true);
			$clinic->flow_title = $this->implode_data($flows, 'flow_title');
			$clinic->flow_text = $this->implode_data($flows, 'flow_text');
		}
		...
		$isEdited = \Services\Factory::get_instance('implant_recommend')->exists_preview($recommend_id);
		$data = \Services\Factory::get_instance('implant_recommend')->get_by_id($recommend_id, $isEdited, $clinics);

		$data->clinics = $clinics;
		$data->site = \Services\Factory::get_instance('site')->get_by_id(2);
	...
	protected function show_create_form()
	{
		...
		if ($request['search_mode'] !== 'import') {
			...
			$recommendId = \Services\Factory::get_instance('implant_recommend')->get_recommend_id($request);
			if ($recommendId > 0) {
				$pathname = $this->_request->getUri()->getPath();
		...
		if ($request['search_mode'] !== 'create') {
			...
			if ($request['attribute_flgname'] !== 'implant_flg') {
	...
	protected function show_preview()
	{
		...
		//プレビューデータの保存
		\Services\Factory::get_instance('implant_recommend')->update($request, true);

		$site = \Services\Factory::get_instance('site')->get_by_id(2);
		$path = \Services\Factory::get_instance('implant_recommend')->get_url_by_id($request['recommend_id']);
	...
	protected function update()
	{
		...
		//データの保存
		\Services\Factory::get_instance('implant_recommend')->update($request, false);
		\Services\Factory::get_instance('implant_recommend')->update($request, true);
	...
	{
		return \Services\Factory::get_instance('implant_clinic')->exists($clinicId);
	}
	...
	private function get_string($request, $type = 'title')
	{
		...
		if ($request['attribute_flgname'] !== 'implant_flg') {
			$str.= $request['attribute_name'].'対応の';
		}
		$str.= 'インプラント治療ができるおすすめ歯医者'.$request['num'].'選';
		$str.= ($type === 'mv_alt') ? 'の画像' : '';
	...
	private function get_lead_string($request, $clinics)
	{
		...
		$str .= ($request['attribute_flgname'] !== 'implant_flg') ? $request['attribute_name'].'対応の' : '';
		$str .= 'インプラント治療ができるおすすめの歯医者さんをご紹介します。';

		$str.= ($request['station_name']!== '') ? $request['station_name'].'周辺で' : $this->get_pref_city_string($request).'で';
		$str .= ($request['attribute_flgname'] !== 'implant_flg') ? $request['attribute_name'].'を受ける' : 'インプラントをする';
	...
	private function insert_clinics($list)
	{
		foreach ($list as $clinic) {
			$clinic->exists = \Services\Factory::get_instance('implant_clinic')->exists($clinic->clinic_id);
			...
			if (!$masterExists) {
				...
				foreach ($clinic->imageList as $key => $value) {
					...
					if (exif_imagetype($clinic->siteUrl.$value)) {
						$image_id = \Services\Factory::get_instance('clinic_image', 'implant')->get_child_alternatekey('clinic', $clinic->clinic_id, 'image');
						$this->get_image_file($clinic->siteUrl.$value, $clinic->clinic_id, $image_id, 'implant');
					}
					}
				}
			} else {
			...
			if (!$clinic->exists) {
				\Services\Factory::get_instance('implant_clinic')->insert($clinic);
				if ($masterExists) {
					foreach ($clinic->imageList as $key => $value) {
						if (\Services\Curl::get_status_code($clinic->siteUrl.$value) == 200) {
						if (exif_imagetype($clinic->siteUrl.$value)) {
	...
	private function insert_recommend($request, &$data)
	{
		...
		$attribute_id = \Services\Factory::get_instance('attribute')->get_id_by_flgname($request['attribute_flgname'], 'implant', $attribute_type);
		...
		$recommend_id = \Services\Factory::get_instance('implant_recommend')->insert($param);

		foreach ($data->clinics as $clinic) {
			$param = ['recommend_id' => $recommend_id, 'clinic_id' => $clinic->clinic_id];
			\Services\Factory::get_instance('implant_recommend_clinic')->insert($param);
		}
