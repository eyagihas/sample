...
/* 矯正歯科ネットプラス おすすめページ */
class KyouseiRecommends extends Base
{
	use \Values\Meta;
	use \Values\KyouseiRecommends;
	...
	protected function show_cms_list()
	{
		...
		$list->data = \Services\Factory::get_instance('kyousei_recommend')->get_cms_list($request,$page,$limit);
		$list->total = \Services\Factory::get_instance('kyousei_recommend')->get_total_count($request);
	...
	protected function show_select_form()
	{
		...
		$list->attributes = \Services\Factory::get_instance('attribute')->get_list(['type'=>'kyousei', 'is_13'=>true, 'is_valid'=>true, 'attribute_type'=>1]);
	...
	protected function show_select_form_type2()
	{
		$list = new \stdClass();
		$list->prefectures = \Services\Factory::get_instance('prefecture')->get_all(['is_domestic'=>true]);
		$list->attributes = \Services\Factory::get_instance('attribute')->get_list(['type'=>'kyousei', 'is_valid'=>true, 'attribute_type'=>2]);
	...
	protected function show_cms_detail()
	{
		...
		$clinics = \Services\Factory::get_instance('kyousei_recommend_clinic')->get_list($recommend_id, false, true);
		foreach ($clinics as $clinic) {
			$features = \Services\Factory::get_instance('kyousei_recommend_clinic_feature')->get_cms_by_clinic_id($recommend_id, $clinic->clinic_id, false);
			$clinic->feature_title = $this->implode_data($features, 'feature_title');
			$clinic->feature_text = $this->implode_data($features, 'feature_text');

			$flows = \Services\Factory::get_instance('kyousei_recommend_clinic_flow')->get_cms_by_clinic_id($recommend_id, $clinic->clinic_id, false);
			$clinic->flow_title = $this->implode_data($flows, 'flow_title');
			$clinic->flow_text = $this->implode_data($flows, 'flow_text');
		}

		$preview_clinics = \Services\Factory::get_instance('kyousei_recommend_clinic')->get_list($recommend_id, true, true);
		foreach ($preview_clinics as $clinic) {
			$features = \Services\Factory::get_instance('kyousei_recommend_clinic_feature')->get_cms_by_clinic_id($recommend_id, $clinic->clinic_id, true);
			$clinic->feature_title = $this->implode_data($features, 'feature_title');
			$clinic->feature_text = $this->implode_data($features, 'feature_text');

			$flows = \Services\Factory::get_instance('kyousei_recommend_clinic_flow')->get_cms_by_clinic_id($recommend_id, $clinic->clinic_id, true);
			$clinic->flow_title = $this->implode_data($flows, 'flow_title');
			$clinic->flow_text = $this->implode_data($flows, 'flow_text');
		}
		...
		$isEdited = \Services\Factory::get_instance('kyousei_recommend')->exists_preview($recommend_id);
		$data = \Services\Factory::get_instance('kyousei_recommend')->get_by_id($recommend_id, $isEdited, $clinics);

		$data->clinics = $clinics;
		$data->site = \Services\Factory::get_instance('site')->get_by_id(3);
	...
	protected function show_create_form()
	{
		...
		if ($request['search_mode'] !== 'import') {
			...
			$recommendId = \Services\Factory::get_instance('kyousei_recommend')->get_recommend_id($request);
		...
		if ($request['search_mode'] !== 'create') {
			$kyouseiList = $this->get_clinic_list('3', 'kyousei', $request);
			if ($request['attribute_flgname'] !== 'child_flg') {
				$implantList = $this->get_clinic_list('2', 'implant', $request);
		...
		if (count($uniqueList) > 0) {
			$clinics = array_slice($uniqueList, 0, $request['num']);
			//新規クリニックはDB登録
			$clinics = $this->insert_clinics($clinics);
		}
		$data->clinics = $clinics;
		...
		$data->lead_default = $this->get_lead_string($request, $clinics);
	...
	protected function show_preview()
	{
		...
		//プレビューデータの保存
		\Services\Factory::get_instance('kyousei_recommend')->update($request, true);

		$site = \Services\Factory::get_instance('site')->get_by_id(3);
		$path = \Services\Factory::get_instance('kyousei_recommend')->get_url_by_id($request['recommend_id']);
	...
	protected function update()
	{
		...
		//データの保存
		\Services\Factory::get_instance('kyousei_recommend')->update($request, false);
		\Services\Factory::get_instance('kyousei_recommend')->update($request, true);
	...
	private function get_hit_clinic($list, $selectedFlg, $siteId) {
		...
		return array_filter($list, function($element) use($selectedFlg, $site) {
	...
	{
		return \Services\Factory::get_instance('kyousei_clinic')->exists($clinicId);
	}
	...
	private function get_station_title_prefix($request)
    {
    	$str = '';

    	$str.= $request['station_name'].'駅周辺';
		$str.= ($request['attribute_flgname'] !== 'ty_adult_orthodontic') ? 'で' : 'の';
	...
	private function get_string($request, $type = 'title')
	{
		...
		$str.= ($request['station_name']!== '') ? $this->get_station_title_prefix($request) : $this->get_pref_city_string($request).'で';
		if ($request['recommend_type'] === '2') {
			$str.= $request['attribute_name'].'取り扱いおすすめ矯正歯科';
		} elseif ($request['attribute_flgname'] !== 'ty_adult_orthodontic') {
			$str.= $request['attribute_name'].'対応のおすすめ矯正歯科';
		} else {
			$str.= 'おすすめ矯正歯科';
		}
		$str.= ($request['recommend_type'] === '2') ? '' : $request['num'].'選';
		$str.= ($type === 'mv_alt') ? 'の画像' : '';
	...
	private function get_lead_string($request, $clinics)
	{
		...
		$str .= ($request['attribute_flgname'] !== 'ty_adult_orthodontic') ?
		$request['attribute_name'].'ができる' : '矯正歯科治療ができる';

		$str .= 'おすすめの歯医者さん';
		$str .= ($request['attribute_flgname'] !== 'ty_adult_orthodontic') ? '（矯正歯科）' : '';
		$str .= 'をご紹介します。';

		$str .= ($request['station_name']!== '') ? $request['station_name'].'駅周辺で' : $this->get_pref_city_string($request).'で';
		$str .= ($request['attribute_flgname'] !== 'ty_adult_orthodontic') ? $request['attribute_name'] : '歯列矯正';
		$str .= 'をするならどこのクリニック？何に注目して選ぶべき？そんなお悩みがある方に本記事はおすすめです。&#10;&#10;';

		$str .= 'この記事で紹介する';
		$str .= ($request['station_name']!== '') ? $request['station_name'].'駅周辺で' : $this->get_pref_city_string($request).'で';
		$str .= ($request['attribute_flgname'] !== 'ty_adult_orthodontic') ? $request['attribute_name'].'に対応している' : '';
		$str .= 'おすすめの矯正歯科医院は下記の通りです。&#10;&#10;';
		...
		$str .= '矯正歯科医院選びの参考として、アクセスや診療時間などの基本情報や医院ごとの特長、院内写真などを掲載しています。';
		$str .= ($request['attribute_flgname'] !== 'ty_adult_orthodontic') ? $request['attribute_name'] : '矯正歯科治療';
		$str .= 'を検討中の方はぜひ読んでみてください。';
	...
	private function get_description_string($request)
	{
		...
		$str .= '矯正歯科医院へのアクセスや診療時間などの基本情報や医院ごとの特長を掲載していますので、';
		$str.= ($request['station_name']!== '') ? $request['station_name'].'駅お近くで' : $this->get_pref_city_string($request).'で';

		$str .= ($request['attribute_flgname'] !== 'ty_adult_orthodontic') ? $request['attribute_name'] : '歯列矯正';
		$str .= 'をお考えの方は是非参考にしてください。';
	...
	private function insert_clinics($list)
	{
		foreach ($list as $clinic) {
			$clinic->exists = \Services\Factory::get_instance('kyousei_clinic')->exists($clinic->clinic_id);
			$masterExists = \Services\Factory::get_instance('clinic')->exists($clinic->clinic_id);
			/* 矯正のみ：静的ページにおいてすでに画像が設置されているかどうか確認 */
			$dir_exists = \Services\Factory::get_instance('clinic_image', 'kyousei')->exists_by_clinic($clinic->clinic_id);
				...
				if (!$dir_exists) {
					foreach ($clinic->imageList as $key => $value) {
						...
						if (exif_imagetype($clinic->siteUrl.$value)) {
							$image_id = \Services\Factory::get_instance('clinic_image', 'kyousei')->get_child_alternatekey('clinic', $clinic->clinic_id, 'image');
							$this->get_image_file($clinic->siteUrl.$value, $clinic->clinic_id, $image_id, 'kyousei');
						}							
						}
					}	
				}
			...
			if (!$clinic->exists) {
				\Services\Factory::get_instance('kyousei_clinic')->insert($clinic);
				if ($masterExists && !$dir_exists) {
					foreach ($clinic->imageList as $key => $value) {
						if (\Services\Curl::get_status_code($clinic->siteUrl.$value) == 200) {
						if (exif_imagetype($clinic->siteUrl.$value)) {
							$image_id = \Services\Factory::get_instance('clinic_image', 'kyousei')->get_child_alternatekey('clinic', $clinic->clinic_id, 'image');
	...
	private function insert_recommend($request, &$data)
	{
		...
		$attribute_type = (int)$request['recommend_type'];
		$attribute_id = \Services\Factory::get_instance('attribute')->get_id_by_flgname($request['attribute_flgname'], 'kyousei', $attribute_type);
		...
		$recommend_id = \Services\Factory::get_instance('kyousei_recommend')->insert($param);

		foreach ($data->clinics as $clinic) {
			$param = ['recommend_id' => $recommend_id, 'clinic_id' => $clinic->clinic_id];
			\Services\Factory::get_instance('kyousei_recommend_clinic')->insert($param);
		}
