...
class TKyouseiRecommendClinics extends Base
{
    use \Modelings\KyouseiRecommendClinics;

    public function __construct()
    {
        parent::__construct();
        $this->_tableName = 't_kyousei_recommend_clinics';
    }
    ...
    public function get_list($recommend_id, $isPreview = false, $isCms = false)
    {
        ...
        try {
            $sub_query = $this->_db->table('t_kyousei_recommend_clinic_features'.$postfix)
                ->select(
                    'recommend_id',
                    'clinic_id',
                ...
                ->where('recommend_id','=',$recommend_id)
                ->groupBy('clinic_id');
            $fee_sub_query = $this->_db->table('t_kyousei_recommend_clinic_fees'.$postfix)
                ->select(
                    'recommend_id',
                    'clinic_id',
                    $this->_db->raw('group_concat(fee_name order by sort_order asc) as fee_name'),
                    $this->_db->raw('group_concat(fee order by sort_order asc) as fee')
                    )
                ->where('recommend_id','=',$recommend_id)
                ->groupBy('clinic_id');
            $flow_sub_query = $this->_db->table('t_kyousei_recommend_clinic_flows'.$postfix)
                ->select(
                    'recommend_id',
                    'clinic_id',
                    $this->_db->raw('group_concat(flow_title order by sort_order asc) as flow_title'),
                    $this->_db->raw('group_concat(flow_text order by sort_order asc) as flow_text')
                    )
                ->where('recommend_id','=',$recommend_id)
                ->groupBy('clinic_id');
            return $this->_db->table($this->_tableName.$postfix.' as rc')
                ->select(
                    ...
                    'fe.fee_name as fee_name',
                    'fe.fee as fee',
                    'flow.flow_title as flow_title',
                    'flow.flow_text as flow_text',
                    'rc.is_deleted as is_deleted',
                ...
                ->join('t_kyousei_clinics as kc', 'rc.clinic_id', '=', 'kc.clinic_id')
                ->leftjoin($this->_db->raw('('.$sub_query->toSql().') as f'),'rc.clinic_id','=','f.clinic_id')
                ->leftjoin($this->_db->raw('('.$fee_sub_query->toSql().') as fe'),'rc.clinic_id','=','fe.clinic_id')
                ->leftjoin($this->_db->raw('('.$flow_sub_query->toSql().') as flow'),'rc.clinic_id','=','flow.clinic_id')
                ->mergeBindings($sub_query)
                ->mergeBindings($fee_sub_query)
                ->mergeBindings($flow_sub_query)
        ...
        try {
            $list = $this->_db->table($this->_tableName.' as rc')
                ->select(
                    ...
                    'a.kyousei_attribute_type as attribute_type'
                )
                ->join('t_kyousei_recommends as r', 'rc.recommend_id', '=', 'r.recommend_id')
    ...
    public function get_detail($recommend_id, $clinic_id, $isPreview = false)
    {
        ...
        try {
            $data = $this->_db->table($this->_tableName.$postfix.' as rc')
                ...
                ->join('t_kyousei_recommends as r','rc.recommend_id', '=', 'r.recommend_id')
                ->join('t_clinics as c','rc.clinic_id', '=', 'c.clinic_id')
                ->join('t_kyousei_clinics as ic','rc.clinic_id', '=', 'ic.clinic_id')
                ->join('m_attributes as a','r.attribute_id', '=', 'a.attribute_id')
                ->where('rc.recommend_id', '=', $recommend_id)
                ->where('rc.clinic_id', '=', $clinic_id)
                ->first();
