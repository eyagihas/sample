...
class TImplantClinics extends Base
{
	use \Modelings\Clinics;

	public function __construct() {
		parent::__construct();
        $this->_tableName = 't_implant_clinics';
	}
  	...
  	public function get_portal_list($recommend_id, $isPreview = false)
 	{
		try {
			...
			$clinics = $this->_db->table($this->_tableName.' as ic')
            		...
            		->join('t_implant_recommend_clinics'.$postfix.' as rc','ic.clinic_id','=','rc.clinic_id')
            		->join('t_implant_recommends'.$postfix.' as r', 'rc.recommend_id', '=', 'r.recommend_id')
  	...
  	{
		try {
			$value = $this->_db->table('t_implant_clinic_images')
					->select('filename')
  	...
  	public function get_cms_list($request = null,$page = null,$limit = null)
    {
        try {
            ...
            foreach ($list as $row) {
                $row->post_num = \Services\Factory::get_instance('implant_recommend_clinic')->get_postnum_by_clinic($row->clinic_id);
                $row->pr_num = \Services\Factory::get_instance('implant_recommend_clinic')->get_postnum_by_clinic($row->clinic_id, true);
            }
