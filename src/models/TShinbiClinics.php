...
class TShinbiClinics extends Base
{
    ...
    {
        parent::__construct();
        $this->_tableName = 't_shinbi_clinics';
    }
    ...
    {
        try {
            return $this->_db->table($this->_tableName.' as sc')
                ->select('c.clinic_id')
                ->join('t_clinics as c', 'sc.clinic_id', '=', 'c.clinic_id')
                ->whereRaw(
                    'case when sc.tel != "" then replace(sc.tel,"-","") = "'.$tel.'" '.
    ...
    public function get_portal_list($recommend_id, $isPreview = false)
    {
        try {
            ...
            $clinics = $this->_db->table($this->_tableName.' as sc')
                    ->select(
                        ...
                        $this->_db->raw('case when sc.gmap_src != "" then sc.gmap_src else c.gmap_src end as gmap_src'),
                        $this->_db->raw('case when sc.access != "" then sc.access else c.access end as access'),
                        $this->_db->raw('case when sc.address != "" then sc.address else c.address end as address'),
                        $this->_db->raw('case when sc.holiday_note != "" then sc.holiday_note else c.holiday_note end as holiday_note'),
                        $this->_db->raw('case when sc.url != "" then sc.url else c.url end as url'),
                        $this->_db->raw('case when sc.url_sp != "" then sc.url_sp else c.url_sp end as url_sp'),
                        $this->_db->raw('case when sc.url_plus != "" then sc.url_plus else c.url_plus end as url_plus'),
                        $this->_db->raw('case when sc.teikei_page_url != "" then sc.teikei_page_url else c.teikei_page_url end as teikei_page_url'),
                        $this->_db->raw('case when sc.reserve_url != "" then sc.reserve_url else c.reserve_url end as reserve_url'),
                        $this->_db->raw('case when sc.reserve_url_plus != "" then sc.reserve_url_plus else c.reserve_url_plus end as reserve_url_plus'),
                        $this->_db->raw('case when sc.tel != "" then sc.tel else c.tel end as tel'),
                        $this->_db->raw('case when sc.tel_sp != "" then sc.tel_sp else c.tel_sp end as tel_sp'),
                        $this->_db->raw('case when sc.reserve_tel != "" then sc.reserve_tel else c.reserve_tel end as reserve_tel'),
                        'sc.is_url_visible as is_url_visible',
                        'sc.is_url_sp_visible as is_url_sp_visible',
                        'sc.is_url_plus_visible as is_url_plus_visible',
                        'sc.is_url_teikei_visible as is_url_teikei_visible',
                        'sc.is_reserve_url_visible as is_reserve_url_visible',
                        'sc.is_reserve_url_plus_visible as is_reserve_url_plus_visible',
                        'sc.is_tel_visible as is_tel_visible',
                        'sc.is_tel_sp_visible as is_tel_sp_visible',
                        'sc.is_reserve_tel_visible as is_reserve_tel_visible',
                        'sc.is_pr_reserve_tel_visible as is_pr_reserve_tel_visible',
                        'sc.is_pr_reserve_url_visible as is_pr_reserve_url_visible',
                    ...
                    ->join('t_clinics as c','sc.clinic_id','=','c.clinic_id')
                    ->join('t_shinbi_recommend_clinics'.$postfix.' as rc','sc.clinic_id','=','rc.clinic_id')
                    ->join('t_shinbi_recommends'.$postfix.' as r', 'rc.recommend_id', '=', 'r.recommend_id')
    ...
    {
        try {
            $value = $this->_db->table('t_shinbi_clinic_images')
                    ->select('filename')
    ...
    public function get_cms_list($request = null,$page = null,$limit = null)
    {
        try {
            ...
            foreach ($list as $row) {
                $row->post_num = \Services\Factory::get_instance('shinbi_recommend_clinic')->get_postnum_by_clinic($row->clinic_id);
                $row->pr_num = \Services\Factory::get_instance('shinbi_recommend_clinic')->get_postnum_by_clinic($row->clinic_id, true);
            }
    ...
    public function get_cms_clinic_name($id)
    {
        try {
            return $this->_db->table($this->_tableName.' as sc')
                ->select(
                    'sc.clinic_id as clinic_id',
                    $this->_db->raw('case when sc.clinic_name != "" then sc.clinic_name else c.clinic_name end as clinic_name')
                )
                ->join('t_clinics as c', 'sc.clinic_id', '=', 'c.clinic_id')
                ->where('sc.clinic_id', '=', $id)->first();
