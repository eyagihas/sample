...
class TKyouseiRecommends extends Base
{
    use \Modelings\KyouseiRecommends;

    public function __construct()
    {
        parent::__construct();
        $this->_tableName = 't_kyousei_recommends';
    }
    ...
    {
        try {
            $result = $this->_db->table('t_kyousei_recommends as r')
    ...
    {
        try {
            $data = $this->_db->table('t_kyousei_recommends as r')
                ->select(
    ...
    public function get_by_id($id, $isEdited = false, $clinics = null)
    {
        ...
        try  {
            $recommend = $this->_db->table($this->_tableName.$postfix.' as r')
                ...
                ->leftJoin('t_kyousei_recommend_images as i','r.recommend_id','=','i.recommend_id')
                ->leftJoin('m_cities as c','r.city_id','=','c.city_id')
                ->leftJoin('m_attributes as a','r.attribute_id','=','a.attribute_id')
                ->leftJoin('m_stations as s','r.station_group_id','=','s.station_group_id')
                ->leftJoin('m_station_groups as sg','r.station_group_id','=','sg.station_group_id')
                ->leftJoin('m_cities as cs','s.city_id','=','cs.city_id')
                ->leftJoin('t_kyousei_recommend_tags'.$postfix.' as rt','r.recommend_id','=','rt.recommend_id')
            ...
            $list = $this->_db->table($this->_tableName. ' as r')
                ...
                ->where(function ($query) use ($request) {
                    ...
                    if (isset($request['type'])) {
                        $query->where('a.kyousei_attribute_type','=',$request['type']);
                    } else {
                        $query->where('a.kyousei_attribute_type','=',1);
                    }
                })
