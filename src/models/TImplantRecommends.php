...
class TImplantRecommends extends Base
{
    use \Modelings\ImplantRecommends;
    ...
    {
        try {
            $result = $this->_db->table('t_implant_recommends as r')
    ...
    {
        try {
            $data = $this->_db->table('t_implant_recommends as r')
                ->select(
    ...
    public function get_by_id($id, $isEdited = false, $clinics = null)
    {
        ...
        try  {
            $recommend = $this->_db->table($this->_tableName.$postfix.' as r')
                ...
                ->leftJoin('m_station_groups as sg','r.station_group_id','=','sg.station_group_id')
                ->leftJoin('m_cities as cs','s.city_id','=','cs.city_id')
                ->leftJoin('t_implant_recommend_tags'.$postfix.' as rt','r.recommend_id','=','rt.recommend_id')
            ...
            $list = $this->_db->table($this->_tableName.' as r')
                ...
                ->where(function ($query) use ($request) {
                    ...
                    if (isset($request['type'])) {
                        $query->where('a.implant_attribute_type','=',$request['type']);
                    } else {
                        $query->where('a.implant_attribute_type','=',1);
                    }
                })
    ...
    public function insert($request)
    {
        ...
        try {
            ...
            \Services\Factory::get_instance('implant_recommend_image')->insert($recommend_id);
            $this->_db->commit();
            return $recommend_id;
        } catch (\Exception $e) {
            $queryLogs = $this->_db->getQueryLog();
    ...
    public function update($request, $isPreview = false)
    {
        ...
        try {
            ...
            \Services\Factory::get_instance('implant_recommend_image')->update_row($request);
            $this->_db->commit();
